...
This parser enforces PGN standard: if something goes wrong, make sure
  castling are with capital letter O, not figure 0 and use the common
  dash - like the two following way to castle: O-O or O-O-O The result
  also has to use the common dash.

Still have to be implemented: promotion and en-passant (both are seen as
  special moves and can be easily implemented with some time)."
